...
import { loadAccent, loadFrontSignOpen, loadLang, loadMapLabels, loadVansEnabled, loadLocationAccepted, loadLocationEnabled, loadRecent, loadSimulatedLocation, loadTheme, pushRecent, saveAccent, saveFrontSignOpen, saveLang, saveMapLabels, saveVansEnabled, saveLocationAccepted, saveLocationEnabled, saveSimulatedLocation, saveTheme, type Accent, type Theme } from '../lib/preferences.ts';
...
import { renderDirectionPill, type Side } from '../ui/direction-pill.ts';
...
export function createApp(root: HTMLElement): void {
  ...
  function render(): void {
    ...
    topbar.update(lang, t(lang, 'appName'), state.settings);
  ...
  async function showMap(route: RouteSummary, detail: RouteDetail): Promise<void> {
    ...
    const { createRouteMap } = await import('../ui/map-view.ts');
